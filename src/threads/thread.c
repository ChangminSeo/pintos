...
thread_exit (void) 
{
  ...
  if (curr->name)
  {
	  printf ("%s: exit(%d)\n", strtok_r(curr->name, " ", &save_ptr), curr->exit_status);
  }
  ...
  while(e != list_end(&curr->mmap_list))
  {
	  ...
	  else if (pagedir_is_dirty(curr->pagedir, spte->uaddr))
	  {
		  file_write_at(spte->file, spte->uaddr, spte->read_bytes, spte->off);
	  }
	  e = list_remove(&spte->mmap_list_elem);
	  free_frame(spte->uaddr);
	  pagedir_clear_page(curr->pagedir, spte->uaddr);
	  delete_spt_elem(spte);
  }

  int fd;
  for (fd = 2; fd < curr->fd_count; fd++)
  {
    if (curr->is_file_opened[fd])
    {
      curr->is_file_opened[fd] = false;
  ...
  list_remove(&curr->child_list_elem);
  /* PJ2 end */
